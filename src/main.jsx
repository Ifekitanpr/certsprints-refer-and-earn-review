 ...
 return <section className="referralpage">
  ...
  {redeem==='withdraw'&&!redeemed&&<div className="overlay"><button className="scrim" onClick={()=>{setRedeem(null);setPayoutMethod('')}}/><aside className="withdrawdrawer">
   ...
   <div className="withdrawdrawerscroll"><form className="redeemform" onSubmit={e=>{e.preventDefault();setRedeemed(true)}}>
    ...
    {payoutMethod&&<div className="methodnotice"><span className={'methodbadge '+payoutMethod.toLowerCase().replaceAll(' ','-')}>{payoutMethod==='Stripe'?<img src={assetPath('/assets/stripe/stripe-wordmark-blurple.svg')} alt="Stripe"/>:<CreditCard/>}</span><div><b>{payoutMethod}</b><p>{payoutMethod==='Stripe'?'Connect or select a verified Stripe payout account. Stripe will handle identity verification and supported settlement currencies.':'Enter the receiving bank details. Required account identifiers adapt to the selected bank country.'}</p></div></div>}
    {payoutMethod==='Stripe'&&<div className="stripeconnect"><div><span className="stripebrand"><img src={assetPath('/assets/stripe/stripe-wordmark-blurple.svg')} alt="Stripe"/></span><div><b>No Stripe account connected</b><p>Connect securely to receive international payouts.</p></div></div><button type="button">Connect Stripe</button></div>}
